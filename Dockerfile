...
RUN mkdir -p /etc/certs/noname.xm4s.net \
 ...
 && openssl req -new -x509 -days 365 -key /etc/certs/noname.xm4s.net/privkey.pem -out /etc/certs/noname.xm4s.net/cert.pem \
 && openssl req -new -x509 -days 365 -key /etc/certs/noname.xm4s.net.local/privkey.pem -out /etc/certs/noname.xm4s.net.local/cert.pem \
